...
function requestOnce(method, url, token, payload) {
...
async function fetchAllIssues(repo, opts, token) {
    const issues = [];
...
async function fetchComments(issue, repo, token) {
...
async function runImport(repoArg, pmRoot, opts) {
    ...
    const existing = opts.dryRun ? new Map() : indexByProvenance(readPmItems(pmRoot));
    let imported = 0;
    let updated = 0;
    let skipped = 0;
...
async function runSync(ctx) {
    ...
    for (const entry of plan) {
        ...
        let current;
        try {
            const { body } = await fetchJSON(`https://api.github.com/repos/${repo}/issues/${entry.number}`, token);
        ...
        try {
            await request("PATCH", `https://api.github.com/repos/${repo}/issues/${entry.number}`, token, JSON.stringify({ state: entry.to }));
...
export function buildExportPlan(items, repo) {
    const repoLc = repo?.toLowerCase();
    const plan = [];
    for (const item of items) {
...
export async function applyExportPlan(plan, repo, token, requestFn) {
    ...
    for (const entry of plan) {
        ...
        try {
            ...
            if (entry.action === "update" && entry.number !== undefined) {
                await requestFn("PATCH", `https://api.github.com/repos/${repo}/issues/${entry.number}`, token, JSON.stringify({ title: p.title, body: p.body, labels: p.labels, state: p.state }));
                updated++;
            }
            else {
                await requestFn("POST", `https://api.github.com/repos/${repo}/issues`, token, JSON.stringify({ title: p.title, body: p.body, labels: p.labels }));
                created++;
            }
        }
        catch (err) {
...
export default defineExtension({
    ...
    activate(api) {
        ...
        // -----------------------------------------------------------------------
        api.registerExporter("github", async (ctx) => {
            const options = ctx.options || {};
        ...
        if (typeof api.registerSearchProvider === "function") {
            api.registerSearchProvider({
                ...
                async query(qctx) {
                    ...
                    const docs = Array.isArray(qctx.documents)
                        ? qctx.documents.map((d) => (d?.metadata ? d.metadata : d))
        ...
        api.hooks.afterCommand((ctx) => {
            ...
            if (!id || !ctx.pm_root)
                return;
            const res = spawnSync("pm", ["--path", ctx.pm_root, "--json", "show", id], { encoding: "utf-8" });
            if (res.status !== 0)
                return;
            let repo;
            try {
